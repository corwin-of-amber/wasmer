...
pub trait EmitterARM64 {
    ...
    fn emit_ldaxr(&mut self, sz: Size, reg: Location, dst: Location) -> Result<(), CodegenError>;
    fn emit_ldaxrb(&mut self, sz: Size, reg: Location, dst: Location) -> Result<(), CodegenError>;
    fn emit_ldaxrh(&mut self, sz: Size, reg: Location, dst: Location) -> Result<(), CodegenError>;
    fn emit_stlxr(
        &mut self,
        sz: Size,
        status: Location,
...
impl EmitterARM64 for Assembler {
    ...
    fn emit_ldaxrb(&mut self, _sz: Size, reg: Location, dst: Location) -> Result<(), CodegenError> {
        match (reg, dst) {
    ...
    fn emit_ldaxrh(&mut self, _sz: Size, reg: Location, dst: Location) -> Result<(), CodegenError> {
        match (reg, dst) {
            (Location::GPR(reg), Location::GPR(dst)) => {
                let reg = reg.into_index() as u32;
                let dst = dst.into_index() as u32;
